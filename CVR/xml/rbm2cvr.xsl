...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:eml="urn:oasis:names:tc:evs:schema:eml" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="eml rbm xsi" xmlns:cdf="NIST_V0_cast_vote_records.xsd" xmlns:rbm="hr-rbm.xsd" xsi:noNamespaceSchemaLocation="../../../RemoteBallotMarking/xsd/hr-rbm.xsd">
	...
	<xsl:template match="/RBM/eml:EML">
	...
	<xsl:template match="eml:Contest">
		<xsl:variable name="contestFragment" select="/RBM/Selections/Selection[(SelectionRaceCode = current()/eml:ContestIdentifier/@IdNumber)]"/>
		<cdf:CVRContest>
			<cdf:ContestId>
				<xsl:value-of select="concat('_', eml:ContestIdentifier/@IdNumber)"/>
			</cdf:ContestId>
			<!-- only catches overvotes for plurality voting -->
			<xsl:variable name="maxMinusIndication" select="eml:MaxVotes - count($contestFragment[Selected != ''])"/>
			<xsl:variable name="TotalIndication" select="count($contestFragment[Selected != ''])"/>
			...
			<xsl:with-param name="contestFragment" select="$contestFragment"/>
	...
	<xsl:template match="eml:Candidate|eml:WriteInCandidate">
		...
		<xsl:param name="contestFragment" />
		<xsl:variable name="currentSelection" select="$contestFragment[SelectionCode = current()/eml:CandidateIdentifier/@IdNumber|current()/eml:ProposalItem/@ReferendumOptionIdentifier|current()/eml:WriteInIdentifier/@IdNumber]" />
		<!-- only emit selection when an indication exists -->
		<xsl:if test="$currentSelection/Selected != '' and $currentSelection/Selected != 'false'">
			<cdf:CVRContestSelection>
				...
				<cdf:SelectionPosition>
					<xsl:if test="name() = 'WriteInCandidate'">
						<cdf:CVRWriteIn>
							<cdf:Text>
								<xsl:value-of select="$currentSelection/WriteInName"/>
							</cdf:Text>
						</cdf:CVRWriteIn>
					</xsl:if>
					...
					<cdf:HasIndication>
						<xsl:choose>
							<xsl:when test="$currentSelection/Selected != '' and $currentSelection/Selected != 'false'">yes</xsl:when>
							<xsl:otherwise>no</xsl:otherwise>
						</xsl:choose>
					</cdf:HasIndication>
					<cdf:IsAllocable>
						<xsl:choose>
							<xsl:when test="$currentSelection/Selected != '' and $isOvervoted = false() and eml:CandidateIdentifier/@IdNumber|eml:ProposalItem/@ReferendumOptionIdentifier">yes</xsl:when>
							<xsl:when test="$currentSelection/Selected != '' and $isOvervoted = false()">unknown</xsl:when>
							<xsl:otherwise>no</xsl:otherwise>
						</xsl:choose>
					</cdf:IsAllocable>
					<cdf:NumberVotes>1</cdf:NumberVotes>
					<xsl:if test="$currentSelection/Selected and $currentSelection/Selected != 'true' and $currentSelection/Selected != 'false'">
						<cdf:Rank>
							<xsl:value-of select="$currentSelection/Selected"/>
						</cdf:Rank>
					</xsl:if>
				</cdf:SelectionPosition>
				<cdf:TotalNumberVotes>
					<xsl:choose>
						<xsl:when test="$currentSelection/Selected != '' and $isOvervoted = false()">1</xsl:when>
						<xsl:otherwise>0</xsl:otherwise>
					</xsl:choose>
				</cdf:TotalNumberVotes>
			</cdf:CVRContestSelection>
