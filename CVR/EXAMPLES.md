...
If a `Mark` was present, but was not captured in the initial CVR, then it would be indicated as a `Mark` with the `Status` literal set to `adjudicated`.
...
Conversely, if a `Mark` was determined to not be a selection for a given contest option, then that set `IsAllocable` to `false`.
...
> Previous versions of the spec used a complex set of statuses that the consumer was required to interpert. By using a single flag, we know if the mark counts, and the statuses can tell us why.
